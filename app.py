...
def generar_datos_cocacola():
    ...
    def get_time(lat1, lon1, lat2, lon2):
        ...
        dist = R * c
        return max(2.0, round((dist / 60.0) + 1, 1)) 
...
def solve_engine(df_pedidos, df_config, use_google, api_key):
    # CORRECCIÓN: Definir variable de UI diferente a la variable de estatus del solver
    status_placeholder = st.empty() 
    status_placeholder.info("⚙️ Iniciando Motor T1 (Skills + Turnos + Tráfico)...")
    ...
    if df_config.empty:
        status_placeholder.error("❌ Falta configuración de Muelles (Skills y Horarios).")
        return pd.DataFrame()
    ...
    for idx, row in df_pedidos.iterrows():
        ...
        t_viaje = row.get('Tiempo_Estimado_Manual_h', 5) 
        ...
        # Asignación Heurística para Demo (Nodos)
        n_orig_id = None
        n_dest_id = None
    ...
    solver_status = solver.Solve(model) # Nombre diferente
    
    results = []
    if solver_status in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
        status_placeholder.success(f"✅ Optimización Completada (Google Maps: {'Sí' if use_google else 'No'})")
        ...
        status_placeholder.error("⚠️ No se encontró solución factible.")
        return pd.DataFrame()
